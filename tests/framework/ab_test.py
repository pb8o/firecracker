...
def git_ab_test(
    ...
    *,
    ...
    bin_dir_a = git_clone(Path("build") / a_revision, a_revision)
    result_a = test_runner(bin_dir_a, True)

    if b_revision:
        bin_dir_b = git_clone(Path("build") / b_revision, b_revision)
    else:
        # By default, pytest execution happens inside the `tests` subdirectory. Pass the repository root, as
        # documented.
        bin_dir_b = Path.cwd().parent
    result_b = test_runner(bin_dir_b, False)
...
def git_ab_test_guest_command(
    ...
    *,
    ...
    bin_dir_a = git_clone(Path("build") / a_revision, a_revision)
    res_a = _, old_out, old_err = test_runner(bin_dir_a)
    bin_dir_b = git_clone(Path("build") / b_revision, b_revision)
    res_b = _, new_out, new_err = test_runner(bin_dir_b)
    the_same = comparator(res_a, res_b)
...
def git_clone(clone_path, commitish):
    ...
    utils.run_cmd("./tools/release.sh --profile release", cwd=clone_path)
    return get_binary("firecracker", workspace_dir=clone_path).parent.resolve()
